...
class tp_widget_recent_tweets extends WP_Widget
{
	...
	public function widget( $args, $instance )
	{
		...
		if ( empty( $instance['consumerkey'] ) || empty( $instance['consumersecret'] ) || empty( $instance['accesstoken'] ) || empty( $instance['accesstokensecret'] ) || empty( $instance['cachetime'] ) || empty( $instance['username'] ) ) {
			echo '<strong>' . __( 'Please fill all widget settings!', THEMENAME ) . '</strong>' . $after_widget;
			return;
		}
		...
		if ( $diff >= $crt || empty( $tp_twitter_plugin_last_cache_time ) ) {
			...
			if ( ! class_exists( 'TwitterOAuth' ) ) {
				if ( ! require_once( TEMPLATEPATH . '/widgets/twitter/twitteroauth.php' ) ) {
					echo '<strong>' . __( "Couldn't find twitteroauth.php!", THEMENAME ) . '</strong>' . $after_widget;
					return;
				}
			}
			...
			$tweets = $connection->get( "https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=" . $instance['username'] . "&count=10" ) or die( __( "Couldn't retrieve tweets! Wrong username?", THEMENAME ) );

			if ( ! empty( $tweets->errors ) ) {
				if ( $tweets->errors[0]->message == 'Invalid or expired token' ) {
					echo '<strong>' . $tweets->errors[0]->message . '!</strong><br />' . __( "You'll need to regenerate it", THEMENAME ) . ' <a href="https://dev.twitter.com/apps" target="_blank">' . __( 'here', THEMENAME ) . '</a>!' . $after_widget;
				}
				else {
		...
		function relative_time( $a )
		{
			...
			if ( is_numeric( $d ) && $d > 0 ) {
				...
				if ( $d < 3 ) {
					return __( "right now", THEMENAME );
				}
				//if less then minute
				if ( $d < $minute ) {
					return floor( $d ) . __( " seconds ago", THEMENAME );
				}
				//if less then 2 minutes
				if ( $d < $minute * 2 ) {
					return __( "about 1 minute ago", THEMENAME );
				}
				//if less then hour
				if ( $d < $hour ) {
					return floor( $d / $minute ) . __( " minutes ago", THEMENAME );
				}
				//if less then 2 hours
				if ( $d < $hour * 2 ) {
					return __( "about 1 hour ago", THEMENAME );
				}
				//if less then day
				if ( $d < $day ) {
					return floor( $d / $hour ) . __( " hours ago", THEMENAME );
				}
				//if more then day, but less then 2 days
				if ( $d > $day && $d < $day * 2 ) {
					return __( "yesterday", THEMENAME );
				}
				//if less then year
				if ( $d < $day * 365 ) {
					return floor( $d / $day ) . __( " days ago", THEMENAME );
				}
				//else return more than a year
				return __( "over a year ago", THEMENAME );
			}
		}
		...
		if ( ! empty( $tp_twitter_plugin_tweets ) ) {
			...
			echo '<script type="text/javascript">jQuery(window).load(function () { jQuery.getScript(\'http://platform.twitter.com/widgets.js\'); });</script>';

			echo   '<div class="twitter-feed">';
			echo   '<div class="tweets" id="twitterFeed">';
			$fctr = '1';
			if ( is_array( $tp_twitter_plugin_tweets ) ) {
				foreach ( $tp_twitter_plugin_tweets as $tweet ) {
					echo '<div><a class="twTime" target="_blank" href="http://twitter.com/' . $instance['username'] . '/statuses/' . $tweet['status_id'] . '"><span>' . relative_time( $tweet['created_at'] ) . '</span></a>' . convert_links( $tweet['text'] ) . '</div>';
			...
			echo '</div>';
			echo '<a href="https://twitter.com/' . $instance['username'] . '" class="twitter-follow-button" data-show-count="false">' . __( 'Follow', THEMENAME ) . ' @' . $instance['username'] . '</a>';
	...
	public function form( $instance )
	{
		...
		echo '
			...
			<p><label>' . __( 'Consumer Secret:', THEMENAME ) . '</label>
			...
			<p><label>' . __( 'Access Token:', THEMENAME ) . '</label>
			...
			<p><label>' . __( 'Access Token Secret:', THEMENAME ) . '</label>
			...
			<p><label>' . __( 'Cache Tweets in every:', THEMENAME ) . '</label>
			...
			<p><label>' . __( 'Twitter Username:', THEMENAME ) . '</label>
			...
			<p><label>' . __( 'Tweets to display:', THEMENAME ) . '</label>
