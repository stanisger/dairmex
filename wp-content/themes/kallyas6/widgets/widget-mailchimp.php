...
class ZN_Mailchimp_Widget extends WP_Widget
{
	...
	function widget( $args, $instance )
	{
		...
		if ( isset ( $_POST['zn_mc_email'] ) ) {
			...
			if ( isset ( $data['mailchimp_api'] ) && ! empty ( $data['mailchimp_api'] ) ) {
				...
				if ( $mcapi->listSubscribe( $list_id, $_POST['zn_mc_email'], $merge_vars ) ) {
					...
					$msg = '<span style="color:#ff0000;"><b>' . __( 'Error:', THEMENAME ) . '</b>&nbsp; ' . $mcapi->errorMessage . '</span>';
				}
			}
		}
		...
		if ( empty( $data['mailchimp_api'] ) ) {
			...
			echo '<p>' . __( 'No mailchimp list selected. Please set your mailchimp API key in the theme admin panel and then configure the widget from the widget options.', THEMENAME ) . '</p>';
...
function zn_do_action()
{
	if ( isset ( $_POST['zn_ajax'] ) ) {
		...
		if ( isset ( $_POST['zn_mc_email'] ) ) {
			...
			if ( isset ( $znData['mailchimp_api'] ) && ! empty ( $znData['mailchimp_api'] ) ) {

				include_once( TEMPLATEPATH . '/widgets/mailchimp/MCAPI.class.php' );
				$api_key = $znData['mailchimp_api'];
