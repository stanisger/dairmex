...
function kallyas_register_required_plugins()
{
	...
	$plugins = array (
		array (
			...
			'source'             => ADMIN_MASTER_ADMIN_DIR . 'plugins/revslider.zip',
			// The plugin source
			'required'           => false,
		...
		array (
			...
			'slug'               => 'CuteSlider',
			// The plugin slug (typically the folder name)
			'source'             => ADMIN_MASTER_ADMIN_DIR . 'plugins/cutesliderwp.zip',
			// The plugin source
			'required'           => false,
			// If false, the plugin is only 'recommended' instead of required
			'version'            => '1.1.1',
	...
	$config = array (
		...
		'strings'          => array (
			...
			'complete'                        => __( 'All plugins installed and activated successfully. %s', THEMENAME ),
			// %1$s = dashboard link
			'nag_type'                        => 'updated'
 ...
 * @since v3.6.5
 *
 * Register the ajax script and action for collecting debug info
 */
add_action('admin_footer', array('ZnDebug', 'enqueueAjaxScript'));
add_action('wp_ajax_ajax_collect_info', array('ZnDebug', 'ajax_collect_info'));
