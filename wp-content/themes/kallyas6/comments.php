...
<div id="respond">
	...
	<?php if ( get_option( 'comment_registration' ) && ! $user_ID ) : ?>
		...
		<p><?php echo __( "You must be ", THEMENAME ); ?><a
				href="<?php echo get_option( 'siteurl' ); ?>/wp-login.php?redirect_to=<?php echo urlencode( get_permalink() ); ?>"><?php echo __( "logged in", THEMENAME ); ?></a> <?php echo __( "to post a comment.", THEMENAME ); ?>
		</p>
	...
	<?php else : ?>
		...
		<form action="<?php echo get_option( 'siteurl' ); ?>/wp-comments-post.php" method="post" id="commentform">
			<?php if ( $user_ID ) : ?>
				...
				<p><?php echo __( "Logged in as", THEMENAME ); ?> <a
						href="<?php echo get_option( 'siteurl' ); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>.
					<a href="<?php echo wp_logout_url( get_permalink() ); ?>"
					   title="Log out of this account"><?php echo __( "Log out", THEMENAME ); ?> &raquo;</a></p>
			...
			<?php else : ?>
				...
				<div class="row-fluid">
					<div class="span4"><input type="text" name="author" id="author"
											  ...
											  class="span12" <?php if ( $req ) {
							echo "aria-required='true'";
						} ?> placeholder="<?php echo __( "Name", THEMENAME ); ?> <?php if ( $req ) {
							echo "(required)";
						} ?>"/>
					</div>
					<div class="span4"><input type="text" name="email" id="email"
							...
							echo "aria-required='true'";
											  ...
											  placeholder="<?php echo __( "Mail (will not be published)", THEMENAME ); ?> <?php if ( $req ) {
												  echo "(required)";
											  } ?>"/>
					</div>
					...
					<div class="span4"><input type="text" name="url" id="url"
											  ...
											  class="span12"
											  placeholder="<?php echo __( "Website", THEMENAME ); ?>"/>
					</div>
				</div>
			<?php endif; ?>
			...
			<p><input name="submit" type="submit" id="submit" tabindex="5"
					  value="<?php _e( "Submit Comment", THEMENAME ); ?>" class="btn"/>
