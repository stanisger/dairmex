...
function _content_sidebar( $options )
{
					...
					<div class="row">
						<?php while ( have_posts() ) : the_post();
							...
							if ( empty ( $post_meta_fields['page_layout'] ) || empty ( $post_meta_fields['sidebar_select'] ) ) {
								if ( $znData['default_sidebar_position'] == 'left_sidebar' ) {
									$content_css  = 'span9 zn_float_right zn_content';
									$sidebar_css  = 'sidebar-left';
									$has_sidebar  = true;
									$mainbody_css = 'zn_has_sidebar';
								}
								elseif ( $znData['default_sidebar_position'] == 'right_sidebar' ) {
							...
							elseif ( $post_meta_fields['page_layout'] == 'left_sidebar' || ( $post_meta_fields['page_layout'] == 'default' && ! empty ( $znData['default_sidebar_position'] ) && $znData['default_sidebar_position'] == 'left_sidebar' ) ) {
								$content_css  = 'span9 zn_float_right zn_content';
								$sidebar_css  = 'sidebar-left';
								$has_sidebar  = true;
								$mainbody_css = 'zn_has_sidebar';
							...
							elseif ( $post_meta_fields['page_layout'] == 'right_sidebar' || ( $post_meta_fields['page_layout'] == 'default' && ! empty ( $znData['default_sidebar_position'] ) && $znData['default_sidebar_position'] == 'right_sidebar' ) ) {
								$content_css  = 'span9 zn_content';
								$sidebar_css  = 'sidebar-right ';
								$has_sidebar  = true;
								$mainbody_css = 'zn_has_sidebar';
								...
								<div class="itemView clearfix eBlog">
									<div class="itemHeader">
										<div class="post_details">
											<span
												class="itemAuthor"><?php echo __( "by ", THEMENAME ); ?><?php the_author_posts_link(); ?></span>
											<span class="infSep"> / </span>
											<span class="itemDateCreated"><span
													class="icon-calendar"></span> <?php the_time( 'l, d F Y' ); ?></span>
											<span class="infSep"> / </span>
											<span class="itemCommentsBlock"></span>
											<span class="itemCategory"><span
													class="icon-folder-close"></span> <?php echo __( 'Published in ', THEMENAME ); ?></span><?php the_category( ", " ); ?>
										</div>
									</div>
									<!-- end itemheader -->
									...
									<?php
										...
										wp_link_pages( array (
											'before' => '<div class="page-link"><span>' . __( 'Pages:', THEMENAME ) . '</span>',
											'after'  => '</div>'
										) );

										if ( ! empty( $post_meta_fields['show_social'] ) && $post_meta_fields['show_social'] == 'show' || ( ! empty( $post_meta_fields['show_social'] ) && $post_meta_fields['show_social'] == 'default' && $znData['show_social'] == 'show' ) ) {
											?>
											<!-- Social sharing -->
						...
						endwhile;
							...
							if ( empty ( $post_meta_fields['page_layout'] ) || empty ( $post_meta_fields['sidebar_select'] ) ) {
								if ( $znData['default_sidebar_position'] == 'left_sidebar' || $znData['default_sidebar_position'] == 'right_sidebar' ) {
									echo '<div class="span3">';
									echo '<div id="sidebar" class="sidebar ' . $sidebar_css . '">';
									if ( ! function_exists( 'dynamic_sidebar' ) || ! dynamic_sidebar( $znData['single_sidebar'] ) ) : endif;
									echo '</div>';
									echo '</div>';
								}
							}
							// WE CHECK IF WE HAVE A SIDEBAR SET IN PAGE OPTIONS
							elseif ( ( ( $post_meta_fields['page_layout'] == 'left_sidebar' || $post_meta_fields['page_layout'] == 'right_sidebar' ) && $post_meta_fields['sidebar_select'] != 'default' ) || ( $post_meta_fields['page_layout'] == 'default' && $znData['default_sidebar_position'] != 'no_sidebar' && $post_meta_fields['sidebar_select'] != 'default' ) ) {
							...
							elseif ( $post_meta_fields['page_layout'] == 'default' && $znData['default_sidebar_position'] != 'no_sidebar' && $post_meta_fields['sidebar_select'] == 'default' || ( ( $post_meta_fields['page_layout'] == 'left_sidebar' || $post_meta_fields['page_layout'] == 'right_sidebar' ) && $post_meta_fields['sidebar_select'] == 'default' ) ) {
								echo '<div class="span3">';
								echo '<div id="sidebar" class="sidebar ' . $sidebar_css . '">';
								if ( ! function_exists( 'dynamic_sidebar' ) || ! dynamic_sidebar( $znData['single_sidebar'] ) ) : endif;
								echo '</div>';
	...
	elseif ( 'page' == get_post_type() ) {
		...
		if ( empty ( $meta_fields['page_layout'] ) || empty ( $meta_fields['sidebar_select'] ) ) {
			if ( $znData['page_sidebar_position'] == 'left_sidebar' ) {
				$content_css  = 'span9 zn_float_right zn_content';
				$sidebar_css  = 'sidebar-left';
				$has_sidebar  = true;
				$mainbody_css = 'zn_has_sidebar';
			}
			elseif ( $znData['page_sidebar_position'] == 'right_sidebar' ) {
				$content_css  = 'span9 zn_content';
				$sidebar_css  = 'sidebar-right';
				$has_sidebar  = true;
				$mainbody_css = 'zn_has_sidebar';
		...
		// WE CHECK IF WE HAVE LEFT SIDEBAR
		elseif ( $meta_fields['page_layout'] == 'left_sidebar' || ( $meta_fields['page_layout'] == 'default' && ! empty ( $znData['page_sidebar_position'] ) && $znData['page_sidebar_position'] == 'left_sidebar' ) ) {
			$content_css  = 'span9 zn_float_right zn_content';
			$sidebar_css  = 'sidebar-left';
			$has_sidebar  = true;
			$mainbody_css = 'zn_has_sidebar';
		...
		// WE CHECK IF WE HAVE RIGHT SIDEBAR
		elseif ( $meta_fields['page_layout'] == 'right_sidebar' || ( $meta_fields['page_layout'] == 'default' && ! empty ( $znData['page_sidebar_position'] ) && $znData['page_sidebar_position'] == 'right_sidebar' ) ) {
			$content_css  = 'span9 zn_content';
			$sidebar_css  = 'sidebar-right ';
			$has_sidebar  = true;
			$mainbody_css = 'zn_has_sidebar';
		...
		while ( have_posts() ) : the_post();
			...
			if ( ! empty( $content ) || ( isset ( $meta_fields['page_title_show'] ) && $meta_fields['page_title_show'] == 'yes' ) )
			{
				...
				if ( ! empty( $znData['zn_enable_page_comments'] ) && $znData['zn_enable_page_comments'] == 'yes' ) {
					?>
					<!-- DISQUS comments block -->
					<div class="disqusForm">
						<?php comments_template(); ?>
				...
				// WE CHECK IF THIS IS NOT A PAGE FROM THE THEME
				if ( empty ( $meta_fields['page_layout'] ) || empty ( $meta_fields['sidebar_select'] ) ) {
					if ( $znData['page_sidebar_position'] == 'left_sidebar' || $znData['page_sidebar_position'] == 'right_sidebar' ) {
						echo '<div class="span3">';
						echo '<div id="sidebar" class="sidebar ' . $sidebar_css . '">';
						if ( ! function_exists( 'dynamic_sidebar' ) || ! dynamic_sidebar( $znData['page_sidebar'] ) ) : endif;
						echo '</div>';
						echo '</div>';
					}
				}
				// WE CHECK IF WE HAVE A SIDEBAR SET IN PAGE OPTIONS
				elseif ( ( ( $meta_fields['page_layout'] == 'left_sidebar' || $meta_fields['page_layout'] == 'right_sidebar' ) && $meta_fields['sidebar_select'] != 'default' ) || ( $meta_fields['page_layout'] == 'default' && $znData['page_sidebar_position'] != 'no_sidebar' && $meta_fields['sidebar_select'] != 'default' ) ) {
				...
				// WE CHECK IF WE HAVE A SIDEBAR SET FROM THEME'S OPTIONS
				elseif ( $meta_fields['page_layout'] == 'default' && $znData['page_sidebar_position'] != 'no_sidebar' && $meta_fields['sidebar_select'] == 'default' || ( ( $meta_fields['page_layout'] == 'left_sidebar' || $meta_fields['page_layout'] == 'right_sidebar' ) && $meta_fields['sidebar_select'] == 'default' ) ) {
					echo '<div class="span3">';
					echo '<div id="sidebar" class="sidebar ' . $sidebar_css . '">';
					if ( ! function_exists( 'dynamic_sidebar' ) || ! dynamic_sidebar( $znData['page_sidebar'] ) ) : endif;
					echo '</div>';
