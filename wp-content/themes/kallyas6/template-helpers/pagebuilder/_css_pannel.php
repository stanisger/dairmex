				...
				if ( isset ( $options['single_css_panel'] ) && is_array( $options['single_css_panel'] ) ) {
					...
					foreach ( $options['single_css_panel'] as $panel ) {
						...
						if ( isset ( $panel['panel_title'] ) && ! empty ( $panel['panel_title'] ) ) {
							...
							echo '<h3 class="title">';
							if(isset($panel['title_link']) && !empty($panel['title_link'])){
								echo '<a href="'.$panel['title_link']['url'].'" target="'.$panel['title_link']['target'].'">' .$panel['panel_title'].'</a>';
							}
							else { echo $panel['panel_title']; }
							echo '</h3>';
