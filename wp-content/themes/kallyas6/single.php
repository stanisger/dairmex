...
<section id="content">
				...
				<div class="row-fluid">
					<?php while ( have_posts() ) : the_post();
						...
						if ( has_post_thumbnail() && ! post_password_required() ) {
							...
							if ( isset( $znData['sb_use_full_image'] ) && $znData['sb_use_full_image'] == 'yes' ) {
						...
						<div class="<?php echo $content_css;
							echo $row_style; ?> post-<?php the_ID(); ?>">
							...
							<div class="itemView clearfix eBlog">
								...
								<?php
									...
									if ( isset ( $meta_fields['content_main_area'] ) && is_array( $meta_fields['content_main_area'] ) &&
                                         ( isset( $meta_fields['page_builder_layout'] ) && $meta_fields['page_builder_layout'] == 'style1' ) ) {
										zn_get_template_from_area( 'content_main_area', $post->ID, $meta_fields, true );
									}

									if ( ! empty( $post_meta_fields['show_social'] ) && $post_meta_fields['show_social'] == 'show' ||
                                         ( ! empty( $post_meta_fields['show_social'] ) && $post_meta_fields['show_social'] == 'default' && $znData['show_social'] == 'show' ) ) {
										?>
										<!-- Social sharing -->
                                ...
                                <?php
                                /*--------------------------------------------------------------------------------------------------
                                    DISPLAY COMMENTS BEFORE PAGE BUILDER
                                --------------------------------------------------------------------------------------------------*/
                                if ( ! empty( $znData['zn_enable_page_comments'] ) && $znData['zn_enable_page_comments'] == 'yes' ) {
                                    if ( isset( $znData['zn_set_location_page_comments'] ) && (int) $znData['zn_set_location_page_comments'] == 0 ) {
                                        ?>
	...
	<?php
		...
		--------------------------------------------------------------------------------------------------*/
		if ( isset ( $meta_fields['content_main_area'] ) && is_array( $meta_fields['content_main_area'] )
             && ( ! isset( $meta_fields['page_builder_layout'] ) || $meta_fields['page_builder_layout'] == 'default' ) ) {
...
<div class="container">
    <?php
    /*--------------------------------------------------------------------------------------------------
        DISPLAY COMMENTS AFTER PAGE BUILDER
    --------------------------------------------------------------------------------------------------*/
    if ( ! empty( $znData['zn_enable_page_comments'] ) && $znData['zn_enable_page_comments'] == 'yes' ) {
        if ( isset( $znData['zn_set_location_page_comments'] ) && (int) $znData['zn_set_location_page_comments'] == 1 ) {
            ?>
            <!-- DISQUS comments block -->
            <div class="disqusForm">
                <?php comments_template(); ?>
